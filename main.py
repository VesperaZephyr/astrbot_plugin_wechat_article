...
支持：
  ...
  4. OneBot 合并转发消息输出：
     ...
     - 节点 2：总结的图（使用 Markdown + KaTeX 渲染）
     - 后续节点：公众号原文的图片解析（正文长图切片）
"""
...
class WeChatArticlePlugin(Star):
    def __init__(self, context: Context, config: AstrBotConfig):
        ...
        self.config = config
        logger.info("[WeChatArticlePlugin] 微信公众号全文精读与公式总结插件 v1.2.0 已加载。")
    ...
    async def auto_detect_wechat(self, event: AstrMessageEvent):
        ...
        urls = WeChatArticleFetcher.extract_urls(event.message_str)
        if not urls:
            return

        target_url = urls[0]
        logger.info(f"[WeChatArticlePlugin] 自动检测到微信文章链接: {target_url}")
        # 拦截事件，避免 AstrBot 默认闲聊 LLM 重复产生冲突回复
        event.stop_event()
        async for res in self._process_article(event, target_url):
            yield res

    async def _process_article(self, event: AstrMessageEvent, url: str):
        """核心业务流程：抓取 -> 公式提取 -> Markdown总结 -> 长图与卡片渲染 -> 合并转发发送"""
        yield event.plain_result("📖 正在抓取微信公众号文章并解析数学公式，请稍候...")
